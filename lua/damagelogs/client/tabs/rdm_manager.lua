...
function PANEL:UpdateReport(index)
	...
	if not self.Reports[index] then
		if report.status ~= RDM_MANAGER_FINISHED or show_finished:GetBool() then
			...
			self.Reports[index].PaintOver = function(slf)
				if slf:IsLineSelected() then
					slf.Columns[2]:SetTextColor(color_white) -- TODO: "color_white" = predefined?
					slf.Columns[3]:SetTextColor(color_white)
					slf.Columns[5]:SetTextColor(color_white)
					slf.Columns[7]:SetTextColor(color_white)
				else
					slf.Columns[2]:SetTextColor(report.adminReport and Color(190, 190, 0) or Color(0, 190, 0))
					slf.Columns[3]:SetTextColor(Color(190, 0, 0))
					slf.Columns[7]:SetTextColor(colors[report.status])

					if report.chat_open then
						slf.Columns[5]:SetTextColor(Color(100 + math.abs(math.sin(CurTime()) * 155), 0, 0))
					else
						slf.Columns[5]:SetTextColor(color_black)
					end
				end
			end

			self.Reports[index].OnRightClick = function(slf)
				TakeAction()
			end
		else
		...
		self.Reports[index].PaintOver = function(slf)
			if slf:IsLineSelected() then
				slf.Columns[2]:SetTextColor(color_white)
				slf.Columns[3]:SetTextColor(color_white)
				slf.Columns[5]:SetTextColor(color_white)
				slf.Columns[7]:SetTextColor(color_white)
			else
...
function Damagelog:DrawRDMManager(x, y)
	...
	if (managerPerm == 2 or managerPerm == 3) and Damagelog.RDM_Manager_Enabled then
		...
		TakeActionB.Think = function(slf)
			slf:SetEnabled(Damagelog.SelectedReport)
		end
		TakeActionB.DoClick = function(slf)
			TakeAction()
		end
		...
		KillerMessage.PaintOver = function(slf, w, h)
			if slf.DisableR then
				surface.SetDrawColor(color_trablack)
		...
		VictimLogsForm.SetExpanded = function()
...
function PANEL:Init()
	...
	for i = 1, 3 do
		...
		function slay.OnChange(sly, val)
			if val then
				for _, otherSlay in ipairs(self.Slays) do
					if otherSlay ~= sly then
						otherSlay:SetValue(0)
					end
				end

				self.NumSlays = sly.Number
